...
const BusLineList = () => {
    ...
    return (
        ...
        >
            ...
            >
                <h3 style={{ fontSize: "1.5rem", color: "#08374b", fontWeight: "bold", marginBottom: "10px" }}>
                    Најди автобус меѓу постојки
                </h3>
                <input
                    type="text"
            ...
            <span>
              {line.name} - {line.bus?.ownerName || "Непознат сопственик"}
                        ...
                        {isLoggedIn && (
                            <span
                                onClick={() => void handleSave(line.id)}
                                style={{ cursor: "pointer", marginLeft: "10px" }}
                            >
                {savedBusLineIds.includes(line.id) ? (
                    <FaBookmark color="#08374b" size={24} />
                ) : (
                    <FaRegBookmark color="#08374b" size={24} />
                )}
              </span>
                        )}
                    </li>
